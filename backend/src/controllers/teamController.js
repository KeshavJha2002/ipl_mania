...
const getBoundaryDetails = async(req, res) => {
    try {
        const teamname = req.params.teamname
        const boundaryData = await pool.query('with cte as (select * from ball_info where non_boundary = 0 and (batsman_run = 6 or batsman_run = 4)) select batter,count(batsman_run) as total_boundaries from cte where batting_team = $1 group by batter order by total_boundaries desc limit 5', [teamname])
